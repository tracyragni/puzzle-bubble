...
let bubbles: string[][] = [];
let currentBubble: { x: number, y: number, color: string, dx: number, dy: number };
let shooterAngle = 0;
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            if (bubbles[row][col] !== '') {
                ...
                context.beginPath();
                context.arc(col * bubbleSize + bubbleSize / 2, row * bubbleSize + bubbleSize / 2, bubbleSize / 2, 0, Math.PI * 2);
                context.fill();
                context.stroke();
            }
        }
...
function moveCurrentBubble() {
    ...
    if (currentBubble.x - bubbleSize / 2 < 0 || currentBubble.x + bubbleSize / 2 > canvas.width) {
...
function matchAndRemoveBubbles(startRow: number, startCol: number): number {
    ...
    while (queue.length > 0) {
        ...
        if (row < 0 || row >= rows || col < 0 || col >= cols || visited[row][col] || bubbles[row][col] !== color) {
            continue;
        }
...
function dropFloatingBubbles() {
    ...
    function isFloating(row: number, col: number): boolean {
        if (row < 0 || row >= rows || col < 0 || col >= cols || bubbles[row][col] === '' || visited[row][col]) {
            return false;
        }
        if (row === 0) {
            return false;
    ...
    for (let row = 1; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            if (bubbles[row][col] !== '' && !visited[row][col] && isFloating(row, col)) {
                bubbles[row][col] = '';
                floatingBubbles++;
            }
        }
